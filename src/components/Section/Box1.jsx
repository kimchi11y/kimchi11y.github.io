const Box1 = () => {
    return (
      <div className="col-span-full lg:justify-center lg:items-center lg:col-span-2 overflow-hidden flex relative p-8 rounded-xl bg-white border border-gray-200">
        <div className="size-fit relative">
          <div className="relative flex items-center text-indigo-700 text-3xl font-bold text-black">
            Hi, I'm Hakim👋
          </div>
          <h2 className="font-semibold text-gray-950 text-xl">Software Developer.</h2>
          <h2 className="font-semibold text-gray-500 text-base lg:pt-4">
            A Second Year Computer Science Student in IIUM, Malaysia
          </h2>
        </div>
      </div>
    );
  };
  
  export default Box1;
  